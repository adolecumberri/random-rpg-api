...
export class HeroGroup {
  ...
  getHerosByAtt_interval: (att_interval: number) => AnyHero[] = (
    att_interval
  ) => {
    return this.heros.filter(
      (hero) => hero.heroStats.curr_att_interval === att_interval
    );
  };
  ...
  getRandomGroup: (n: number) => AnyHero[] = (n) => {
    ...
    return Array.from(
      Array(n),
      () => copy.splice(Math.floor(copy.length * Math.random()), 1)[0]
    );
  };

  //cojo index y elimino al soldado
  heroDeath: (id: number, turn: number, killedBy: number) => void = (
    id,
    turn,
    killedBy
  ) => {
    let indexToRemove = this.heros.findIndex(
      (hero) => hero.heroStats.id === id
    );
    if (indexToRemove !== -1) {
  ...
  saveHeros: (id_fight: number, turn: number) => Promise<boolean> = async (
    id_fight,
    lastFightturn
  ) => {
    ...
    this.heros.forEach(
      ({ fightStats: { getParamsToInsert }, heroStats }, i) => {
  ...
  saveDeaths: (id_fight: number, turn: number) => Promise<boolean> = async (
    id_fight,
    lastFightturn
  ) => {
    let query = `INSERT INTO groupfightstats 
