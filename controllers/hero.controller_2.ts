...
connection.query(
  `SELECT class.id, name, hp, dmg, def, crit, critDmg, accuracy, evasion, att_interval, reg 
...
let randName = (gender: number) => {
  return (
    "" +
    (gender === 1
      ? hombres[rand(0, hombres.length)]?.nombre
      : mujeres[rand(0, mujeres.length)]?.nombre) +
    " " +
...
let getStats = (baseStats: any, choosedClassStats: any) => {
  ...
  Object.keys(basicStats).map((a: string | any) => {
    ...
    obj[a] =
      Math.round(
        (Math.random() * (value * 1.15 - value * 0.85) + value * 0.85) * 100
      ) / 100;
  });
  return obj;
};
