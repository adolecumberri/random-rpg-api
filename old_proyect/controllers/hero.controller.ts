...
let createHeros = async (req: Request, res: Response) => {
  ...
  for (let i = 0; i < Number(numberToCreate); i++) {
    promises.push(saveHero(createHero())); //saveHero devuelve promesas. Promise.all espera a que se ejecuten todas.
  }
  await Promise.all(promises);
  res.send("Antes aquí se creaban Heroes");
};

let saveHero = async (hero: any) => {
