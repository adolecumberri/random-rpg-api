...
CREATE TABLE `event_journal` (
  ...
  action ENUM("FIGHTING", "MOVING", "STAYING"),
  id_action int default null,
  CONSTRAINT `event_journal_ibfk_1` FOREIGN KEY (`id_event`) REFERENCES `events` (`id`)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci;
/*
 Tabla relacionada con los movimientos de los equipos por un mapa.
 id_crew -> equipo.
 origin -> map_location.id
 destiny -> map_location.id
 */
create table groupmoving (
  ...
  CONSTRAINT `id_crew_moving_fk` FOREIGN KEY (`id_crew`) REFERENCES `crew` (`id`),
  CONSTRAINT `id_origin_fk` FOREIGN KEY (`origin`) REFERENCES `map_locations` (`id`),
  CONSTRAINT `id_destiny` FOREIGN KEY (`destiny`) REFERENCES `map_locations` (`id`)
)